...
// new flag credit true ya false agar paid hova tu chat start kar sakta ha nahi tu nahi kar sakta

export async function createMessage(
  recipientUserId: string,
  data: MessageSchema
): Promise<ActionResult<MessageDto>> {
  // invide ki id  or jis na message bejha ha matlab login user ki id
  try {
    ...
    const message = await prisma.message.create({
      data: {
        text,
        recipientId: recipientUserId, // recipitent user id is the invide id from the top query  recepitent is missing get recipetiend form the invite id this is bascially invide id which we called as recipient id
        senderId: userId,
      },
